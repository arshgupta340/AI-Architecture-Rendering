...
def tag_regions(
    ...
    instruction = (
        ...
        "Downstream, each bbox is fed to SAM2 to produce a pixel mask, then "
        "the masked region is re-rendered with a new material. Bbox quality "
        "directly determines edit quality.\n\n"
        ...
        "ALL bounding boxes MUST be in normalized coordinates of IMAGE 2.\n\n"
        "COORDINATE SYSTEM (strict): use the standard Gemini 0–1000 normalized "
        "space relative to IMAGE 2. x=0 is the left edge, x=1000 is the right "
        "edge; y=0 is the top edge, y=1000 is the bottom edge. The caller "
        "will rescale to actual pixels. Do NOT emit pixel coordinates.\n\n"
        ...
        "  - bbox: {x, y, w, h} in 0–1000 normalized coords "
        "(x, y is top-left; w, h are width and height; x+w ≤ 1000, y+h ≤ 1000)\n"
        "  - confidence: float in [0, 1]\n"
        "  - parent_id: optional id of an enclosing region, or null\n\n"
        "BBOX TIGHTNESS RULES:\n"
        "  - One bbox per ARCHITECTURAL ELEMENT, not per facade. A facade is "
        "    NOT a wall — it is a collection of walls, windows, and mullions. "
        "    Emit a separate 'wall' bbox for each contiguous opaque wall "
        "    SURFACE between openings. A bbox that contains windows inside it "
        "    is too loose for 'wall' — split it.\n"
        "  - Bboxes must be TIGHT: edges should touch the element's visible "
        "    extent. Do not pad. Do not round up to neighboring elements.\n"
        "  - For repeated elements (e.g., 12 identical windows on a facade), "
        "    emit one bbox per visible instance, not a single bbox covering "
        "    the row. Cap at ~30 instances per element type if dense.\n\n"
        "PARENT_ID SEMANTICS (strict):\n"
        "  - parent_id encodes GEOMETRIC CONTAINMENT: child's bbox is fully "
        "    inside parent's bbox AND the child is a real physical sub-part "
        "    of the parent (mullion inside window; door handle inside door).\n"
        "  - parent_id does NOT mean 'spatially overlaps' or 'is near'. If "
        "    two regions merely overlap, leave parent_id null on both.\n"
        "  - Architecturally invalid containments are forbidden: a door is "
        "    NEVER a child of a window; a window is NEVER a child of a door; "
        "    a wall is NEVER a child of a window.\n\n"
        "LABEL DISCIPLINE:\n"
        "  - 'door': a real hinged or sliding human-passage opening at "
        "    ground level or terrace level. NOT balcony glazing, NOT a "
        "    floor-to-ceiling window, NOT a window on an upper floor.\n"
        "  - 'mullion': a vertical or horizontal divider WITHIN a window "
        "    assembly. Emit when visible; otherwise omit (do not invent).\n"
        "  - 'wall' vs 'roof': roof is the topmost horizontal/sloped surface; "
        "    wall is vertical. Do not label a roof as a wall.\n"
        "  - Only tag elements on the PRIMARY building of interest. If "
        "    neighboring buildings are visible, you may tag them, but do not "
        "    invent labels for them that wouldn't fit the primary building.\n"
        "  - If a region does not fit any allowed label, omit it. Do not "
        "    guess. Lower-confidence guesses degrade downstream segmentation.\n\n"
        "Cover every visually distinct architectural element on the primary "
        "building. Prefer many tight bboxes over a few loose ones."
    )
