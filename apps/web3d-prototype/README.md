...
- **Material swap** — click an element → pick a CC0 PBR swatch. UVs are **box-projected world-space in feet**, so texture scale is metric-consistent; live per-material scale slider. Non-destructive **layer stack**, localStorage-persisted.
- **Real glass** — windows share one `MeshPhysicalMaterial` (transmission).
- **Sky & Sun** — real `suncalc` solar position from lat/long/date/time + mood/time/cloud presets.
- **Entourage** — place Tree/Bush/Person (procedural placeholders, real-ft height sliders).
...
- In-app **path tracer crashes** (`three-gpu-pathtracer` reads `.r` of an undefined material color; geometry's dequantized in `public/model/house_pt.glb`, materials need a path-trace-safe color).
- Entourage + materials are **minimal** — expanding both (real assets, KTX2, scale-aware tiling) is the next push (see `../../docs/HANDOFF-web3d.md` §Next-steps).
