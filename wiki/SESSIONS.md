...
**Outcome:** Branch `build/reve-canvas`. Superseding DECISIONS entry (kept the kill note with a forward-pointer — honest reject→confound→retest→reverse record). Rewrote `spike/REPORTS/reve_spike.md` to CONDITIONAL PASS; updated cost ledger ($3.49 running total) + STATE Track 2. New spike scripts: `run_edit_mechanic.py`, `run_edit_primitives.py`, `run_gate_closeout.py`. Interior fixture wired (`spike/reve/fixtures/interior.png`).
**Follow-ups:** run the ≤$1 confirmation battery (interior decomposition FIRST, facet isolation, framing pin); if interior returns a room-blob, re-scope to exteriors-first. Then build `packages/arch-taxonomy` + `apps/reve-canvas` thin vertical slice (edit-from-original invariant; masked-delta composite). Note PRD §4 needs the ~$0.21/edit (2-call) economics correction.
