...
## 2026-07-16 — Reve C2 kill OVERTURNED: mechanism error found; conditional pass {#reve-canvas-revived}

**Decision:** Overturn [[#reve-canvas-killed]] to a **CONDITIONAL PASS** and proceed to build a thin vertical slice. The prior kill rested on a wrong-primitive confound, not a real Reve limitation. Full build is gated on a ≤$1 confirmation battery (interior granularity + facet isolation + framing pin). **Supersedes [[#reve-canvas-killed]].**
**Context:** Re-running the $5 spike (2026-07-16) showed the prior session tested the wrong edit primitive. It edited a region's `prompt` and passed it to `render_layout` with the source image as reference — which **structurally over-preserves** (the reference pixels dominate → the material *cannot* change → "no material change" was guaranteed by the method, not evidence about the product). The documented edit path is Reve's `create_layout` **`change` command**: `extract_layout → create_layout(references=[{image, layout}], commands=[{op:"change", label, new_description}]) → render_layout(edited_layout, ref=image)`. Run live, this swapped a house's dark shingle siding to cream travertine cladding while keeping every window/mullion/roofline/porch/stair pixel-locked — **1.34% drift outside the building bbox, no warping, custom RegionKey labels round-tripped verbatim** (evidence: `spike/reve/outputs/EP_E_comparison.png`, `reve_spike.md`). The prior spike was rigorous; it measured exactly what it set up — it just aimed at the wrong API surface.
**Two reframed findings (Opus advice, this session):**
- **Reframing is not a killer.** Because Reve Canvas is a non-destructive layer editor, it never ships Reve's canvas as the final composite — it lifts the **masked delta** of the edited region and composites it back onto the untouched source screenshot. So framing changes outside the edited region are irrelevant by construction; "reframing" collapses to a registration problem (pin layout width/height to source aspect + pass source as reference + homography-align, then lift the masked region).
- **Region model is object-level, not surface-level → the pitch shifts from "click any surface" to "click an object, edit its material facets."** `extract_layout` returns objects (`<house>`, `<window>`, `<porch>`, `<ground>`, `<sky>`); wall/roof/trim are *clauses inside the building region's description*, edited via targeted `change` descriptions. Extraction granularity is **non-deterministic and prompt-steerable** (some runs surface a separate `<roof>`/`<plain>` wall region). Surface-level clicking is still offerable via SAM sub-segmentation, but the SAM mask directs the *composite-back*, not Reve.
**Architectural invariant this establishes:** **every layer edits from the ORIGINAL image + one change-command, never chains Reve outputs** — this makes drift non-compounding by design and turns render_layout's over-preservation into the region-lock mechanism.
**Alternatives considered:** (a) accept the kill / build only the mesh track — rejected: the kill was a false negative; the product's real edit path works. (b) Hard PASS and build the full app now — rejected: the positive is n=1 on the most favorable case (exterior, whole-building recladding); interiors are the untested high-value unknown. (c) Overwrite the kill record — rejected: the honest record is reject→confound→re-test→reverse, in the open.
**Revisit if:** the confirmation battery's interior shot returns one room-blob (→ re-scope to exteriors-only before building the general UX); facet isolation fails (change-command can't touch the wall clause without moving the roof → surface-clicking isn't honest); or per-edit cost/latency (~$0.21/edit round, 2 calls, ~35s) proves too heavy for a direct-manipulation tool at scale.

## 2026-07-15 — Kill Reve Canvas after geometry-preservation gate failed {#reve-canvas-killed}

> **SUPERSEDED 2026-07-16 by [[#reve-canvas-revived]]** — this kill was a false negative from testing the wrong edit primitive (`render_layout` prompt-rewrite, which over-preserves) instead of the `create_layout` `change` command. Kept for the honest record; see the superseding entry.
