...
**Status: STOPPED at Phase 0 — C2 kill gate failed** ([[DECISIONS#reve-canvas-killed]]; [full report](../spike/REPORTS/reve_spike.md)). Four successful live calls spent 320 credits ($0.4267 exact pack value; Reve balance 7,500→7,180). Strict C1 also missed: 66.7% exterior / 70.0% viewport baseline matcher rates; a targeted architectural refinement recovered roof + siding but reached 77.8% vs the required 80%. The keyed wall→travertine render measured 7.1% change outside the edited bbox vs the <5% C2 threshold, reframed 1504×656 input to 5440×3072, shifted composition, and did not visibly replace the shingle siding. C6 label round-trip passed once; C4/C5 were intentionally not run after the terminal C2 result. `apps/reve-canvas/` and Supabase were **not scaffolded**. Preserve the transferable taxonomy/layout evidence; product effort redirects to the mesh-first geometry-locked track.
