...
- **In-app path tracer crashes at runtime** (`three-gpu-pathtracer` `MaterialsTexture.updateFrom` reads `.r` of an undefined material color) — geometry was fixed (dequantized `public/model/house_pt.glb`), but a material (likely the transmission glass) needs a path-trace-safe color. Task chip spawned. Also: WebGPU mode IBL is reduced for now (node-safe color sky; a node-safe HDRI env is the follow-up so its reflections match the WebGL2 modes).
