...
2c. **Hero polish remaining:** **multi-view-consistent hero** (orbit/saved-view capture → same-seed renders → a strip that feeds the splat bake) and **region_edit v2 true inpaint on FLUX.1** — both scoped, deferred (need live multi-render / a new pipeline to verify).
